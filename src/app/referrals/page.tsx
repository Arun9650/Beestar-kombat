...
const ReferralPage = () => {
  ...
  const handleTap = () => {
    setIsTapped(true);
    navigator.clipboard.writeText(`https://beestar-kombat-omega.vercel.app/?referredByUser=${id}`);
    ...
    <div className="px-4 flex-grow bg-black bg-opacity-60 backdrop-blur-none rounded-t-3xl top-glow border-t-4 border-[#f3ba2f]  text-white  flex flex-col gap-16 ">
     ...
     <p className="mb-4  flex justify-center mt-4">You and your friend will receive bonuses</p>
      ...
      <div className='flex  gap-3 '>
          ...
          {id ? (
        <TelegramShareButton url={`https://beestar-kombat-omega.vercel.app/?referredByUser=${id}`}>
          Invite a friend
        </TelegramShareButton>  
      ) : (
